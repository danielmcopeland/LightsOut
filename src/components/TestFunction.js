...
const TestFunction = props => {
  ...
  const red1 = 'R75,D30,R83,U83,L12,D49,R71,U7,L72';
  const blue1 = 'U62,R66,U55,R34,D71,R55,D58,R83';
  const red2 = 'R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51';
  const blue2 = 'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7';

  const getAnswer = (red, blue) => {
    const blueArr = blue.split(',');
    ...
    for (let i = 0; i < redArr.length; i++) {
      const direction = redArr[i].substring(0, 1);
      const wireLength = parseInt(redArr[i].substring(1));

      switch (direction) {
        case 'U':
          for (let j = 0; j < wireLength; j++) {
            currentLoc = [currentLoc[0], currentLoc[1] + 1];
            wireGrid.push(currentLoc);
          }
          break;
        case 'D':
          for (let j = 0; j < wireLength; j++) {
    ...
    for (let i = 0; i < blueArr.length; i++) {
      const direction = blueArr[i].substring(0, 1);
      const wireLength = parseInt(blueArr[i].substring(1));

      switch (direction) {
        case 'U':
          for (let j = 0; j < wireLength; j++) {
            blueLoc = [blueLoc[0], blueLoc[1] + 1];
            blueGrid.push(blueLoc);
          }
          break;
        case 'D':
          for (let j = 0; j < wireLength; j++) {
    ...
    for (let i = 0; i < wireGrid.length; i++) {
      if (i % 100 == 0) {
        console.log(i);
      }
      for (let j = 0; j < blueGrid.length; j++) {
        if (
          wireGrid[i][0] == blueGrid[j][0] &&
          wireGrid[i][1] == blueGrid[j][1]
        ) {
          crossedWires.push([i, j]);
        }
      }
    }
    console.log(crossedWires);
    const sortedWires = crossedWires.sort((a, b) => {
      const asum = Math.abs(a[0]) + Math.abs(a[1]);
      const bsum = Math.abs(b[0]) + Math.abs(b[1]);
      return asum - bsum;
    });
    console.log(sortedWires[0]);
  };

  return (
    <div>
      <button onClick={() => getAnswer(red1, blue1)}>Test1</button>
      <button onClick={() => getAnswer(red2, blue2)}>Test2</button>
      <button onClick={() => getAnswer(red3, blue3)}>TestBig</button>
    </div>
  );
};
